import { useState } from "react";
import { useForm } from "react-hook-form";
import toast from "react-hot-toast";
import { Edit } from "tabler-icons-react";

import { Dialog, DialogCloseButton } from "~components/Dialog";
import { Label } from "~components/Label";
import { EditSendingProps, useEditSendingMutation } from "~utils/api/sends/editSending";
import { ISending } from "~utils/api/types";

type Props = {
  sending: ISending;
};

export const EditSending = ({ sending }: Props) => {
  const [dialogOpen, setDialogOpen] = useState(false);

  const { mutateAsync } = useEditSendingMutation();

  const { games } = sending;

  const form = useForm({
    defaultValues: {
      gameIds: games?.map((game) => game.id) ?? [],
    },
  });

  const onSubmit = async (values: EditSendingProps["updateData"]) => {
    await toast.promise(mutateAsync({ sendingId: sending.id, updateData: values }), {
      success: "Changes saved",
      error: "Error saving changes",
      loading: "Saving changes",
    });

    setDialogOpen(false);
    form.reset();
  };

  return (
    <Dialog
      open={dialogOpen}
      setOpen={setDialogOpen}
      title="Edit sending"
      trigger={
        <button className="btnBase px-2 hover:bg-gray-700/80 active:bg-gray-700/60">
          <Edit strokeWidth={1.3} size={21} />
        </button>
      }
    >
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-3">
        <div className="flex flex-col gap-2">
          <Label required htmlFor="games">
            Select games
          </Label>

          <select
            id="games"
            className="focus appearance-none rounded-md bg-gray-600 p-2"
            defaultValue={[]}
            multiple
            required
            {...form.register("gameIds")}
          >
            {sending.games?.map((game) => (
              <option key={game.id} value={game.id}>
                {game.name}
              </option>
            ))}
          </select>
        </div>

        <div className="flex items-center justify-between gap-2">
          <DialogCloseButton className="btnBase bg-gray-600 hover:bg-gray-500/80 active:bg-gray-400/60">
            Cancel
          </DialogCloseButton>

          <button
            type="submit"
            className="btnBase border-[1px] border-blue-500 bg-blue-800/60 hover:bg-blue-700/80 active:bg-blue-600/80"
          >
            Save
          </button>
        </div>
      </form>
    </Dialog>
  );
};
